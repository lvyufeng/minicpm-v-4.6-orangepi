...
int main() {
    ...
    for (int64_t i = 0; i < N * H; ++i) {
        float v = static_cast<float>((i * 17 + 5) % 41) / 41.0f - 0.5f;
        x_f[i] = v * 0.5f;
        x_host[i] = f2h(x_f[i]);
    }
    ...
    try {
        rms_norm1024(X, G, Y, static_cast<double>(eps), ctx.stream());
    } catch (const std::exception& e) {
        std::cerr << "rms_norm1024 failed: " << e.what()
    ...
    std::cout << "rms_norm1024 errors=" << errors
              << " max_abs=" << max_abs
              << " (N=" << N << " H=" << H << ")" << std::endl;
    if (errors) return 2;
    std::cout << "rms_norm1024 ok" << std::endl;
    return 0;
}
