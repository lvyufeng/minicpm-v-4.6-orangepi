...
static double bench_cube(int N, int K, int iters, aclrtStream stream) {
...
int main() {
    ...
    std::printf("%-8s %-10s %-10s %-10s %-10s\n", "N", "vec_ms", "cube_ms", "aclnnMm", "cube/Mm");
    for (int N : Ns) {
        int iters = (N >= 100000) ? 5 : 30;
        double v = bench_custom(N, 1024, iters, ctx.stream());
        double c = bench_cube(N, 1024, iters, ctx.stream());
        double r = bench_ref(N, 1024, iters, ctx.stream());
        std::printf("%-8d %-10.3f %-10.3f %-10.3f %-10.2fx\n", N, v, c, r, r / c);
    }
    return 0;
}
