...
int main() {
    ...
    for (int step = 0; step < NumGenerate; ++step) {
        ...
        for (int layer = 0; layer < NumLayers; ++layer) {
            ...
            check_acl(aclrtSynchronizeStream(ctx.stream()), "decode layer sync");
        }
        ...
        int32_t new_tok = static_cast<int32_t>(host_pred.back());
        if (new_tok < 0 || new_tok >= vocab) {
            std::cerr << "generated token out of vocab range: " << new_tok << std::endl;
            return 1;
        }
        tokens.push_back(new_tok);
        std::cout << "step " << step << " new_tok=" << new_tok
                  << " seq=";
        for (auto t : tokens) std::cout << t << ",";
        std::cout << std::endl;
    }

    std::cout << "autoregressive smoke ok generated=" << NumGenerate
              << " final_len=" << tokens.size() << std::endl;
    return 0;
}
