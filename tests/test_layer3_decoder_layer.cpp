...
int main() {
    ...
    constexpr int64_t Intermediate = 3584;
    ...
    Tensor post_norm_w = index.load_to_device_as("model.language_model.layers.3.post_attention_layernorm.weight", DType::Float16);
    ...
    Tensor gate_w = index.load_to_device_as("model.language_model.layers.3.mlp.gate_proj.weight", DType::Float16);
    Tensor up_w = index.load_to_device_as("model.language_model.layers.3.mlp.up_proj.weight", DType::Float16);
    Tensor down_w = index.load_to_device_as("model.language_model.layers.3.mlp.down_proj.weight", DType::Float16);
    ...
    if (q_w.shape() != std::vector<int64_t>{QProjOut, Hidden} ||
        ...
        gate_w.shape() != std::vector<int64_t>{Intermediate, Hidden} ||
        up_w.shape() != std::vector<int64_t>{Intermediate, Hidden} ||
        down_w.shape() != std::vector<int64_t>{Hidden, Intermediate}) {
    ...
    FullAttentionDecoderLayerWeights weights{
        &input_norm_w,
        &post_norm_w,
        &q_w,
        &k_w,
    ...
    FullAttentionDecoderLayerConfig config{
        NumQHeads,
        NumKVHeads,
        HeadDim,
        Rot,
    ...
    Tensor final_hidden({T, Hidden}, DType::Float16);
    final_hidden.allocate();

    full_attention_decoder_layer(hidden, weights, cos_t, sin_t, row_to_t, config, final_hidden, ctx.stream());

    float final0 = 0.0f;
    if (!all_finite_nonzero(final_hidden, &final0)) {
        std::cerr << "layer3 decoder layer smoke produced invalid output" << std::endl;
        return 2;
    }

    std::cout << "layer3 decoder layer smoke ok"
              << " final=[" << final_hidden.shape()[0] << "," << final_hidden.shape()[1] << "]"
              << " first=" << final0 << std::endl;
    return 0;
}
