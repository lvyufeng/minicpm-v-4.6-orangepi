...
int main() {
    ...
    for (int layer = 0; layer < NumLayers; ++layer) {
        ...
        if (ltype == "linear_attention") {
            ...
            Tensor q_w = load_layer_weight(index, layer, "self_attn.q_proj.weight");
            Tensor k_w = load_layer_weight(index, layer, "self_attn.k_proj.weight");
            Tensor v_w = load_layer_weight(index, layer, "self_attn.v_proj.weight");
            Tensor o_w = load_layer_weight(index, layer, "self_attn.o_proj.weight");
            Tensor q_norm_w = load_layer_weight(index, layer, "self_attn.q_norm.weight");
            Tensor k_norm_w = load_layer_weight(index, layer, "self_attn.k_norm.weight");
        ...
        check_acl(aclrtSynchronizeStream(ctx.stream()), "lm layer sync");
    }
    ...
    float first = 0.0f;
    if (!all_finite_nonzero(hidden, &first)) {
        std::cerr << "full language model smoke produced invalid hidden" << std::endl;
        return 1;
    }
    std::cout << "full language model smoke ok"
              << " layers=" << NumLayers
              << " hidden=[" << T << "," << Hidden << "]"
              << " first=" << first
              << " pred=";
    for (auto p : host_pred) std::cout << p << ",";
    std::cout << std::endl;
    return 0;
}
